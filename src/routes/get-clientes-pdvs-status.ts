...
export async function getClientesPdvsStatus(app: FastifyInstance) {
  app.withTypeProvider<ZodTypeProvider>().get(
    ...
    async (request, reply) => {
      ...
      // remover itens undefined retornados pelo map async
      const clientesProcessadosNonNull = clientesProcessados.filter(
        (item) => item !== undefined
      );

      // Aplicar filtros adicionais
      const clientesFiltrados = clientesProcessadosNonNull.filter((cliente) =>
...
const aplicarFiltros = (
  ...
  if (filtraDistro) {
    // defensive: cliente might be undefined or not have distro_linux
    if (!cliente) return false;
    const distro_linux = cliente.distro_linux ?? {
      qtd_centos6: 0,
      qtd_centos7: 0,
      qtd_ubuntu16: 0,
      qtd_ubuntu20: 0,
